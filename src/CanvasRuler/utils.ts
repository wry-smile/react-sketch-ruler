import type { RulerOptions, Shadow } from '../types/index'
...
export const drawHorizontalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: Shadow, options: RulerOptions) => {
  if (!ctx) return
...
export const drawVerticalRuler = (ctx: CanvasRenderingContext2D, start: number, shadow: Shadow, options: RulerOptions) => {
  if (!ctx) return
  const { scale, width, height, canvasConfigs } = options
  const { bgColor, fontColor, shadowColor, ratio, longFGColor, shortFGColor } = canvasConfigs
