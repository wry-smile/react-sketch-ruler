import { memo, useEffect } from 'react'
import { createPortal } from 'react-dom'
import type { RulerContextMenuProps } from './types/index'
import { StyleMenu } from './styles'
import { i18nObj } from './constant'

const SELECT_SVG = <svg width="10" height="10" xmlns="http://www.w3.org/2000/svg"><path d="M1 5.066c0 .211.07.39.212.538L3.346 7.78A.699.699 0 0 0 3.872 8a.69.69 0 0 0 .517-.221l4.39-4.49A.731.731 0 0 0 9 2.753a.717.717 0 0 0-.22-.532A.714.714 0 0 0 8.255 2a.714.714 0 0 0-.524.221l-3.86 3.955L2.26 4.528a.714.714 0 0 0-.524-.221.714.714 0 0 0-.524.221.749.749 0 0 0-.212.538z" fill="#415058" fillRule="evenodd" /></svg>

const RulerContextMenu = (props: RulerContextMenuProps) => {
  const el = document.createElement('div')

  const closeMenu = () => {
    const { onCloseMenu } = props
    onCloseMenu()
  }

  const closeMenuMouse = (event: MouseEvent) => {
    if (event.button !== 2) return
    closeMenu()
  }

  const onHandlerShowRuler = () => {
    const { handleShowRuler } = props
    handleShowRuler()
  }

  const onHandleShowReferLine = () => {
    const { handleShowReferLine } = props
    handleShowReferLine()
  }

  const onHandleShowSpecificRuler = () => {
    const { horLineArr, verLineArr, handleLine, vertical } = props
    const newLines = vertical ? { h: horLineArr, v: [] } : { h: [], v: verLineArr }
    handleLine(newLines)
    closeMenu()
  }

  useEffect(() => {
    document.body.appendChild(el)
    document.addEventListener('click', closeMenu)
    document.addEventListener('mousedown', closeMenuMouse)
  }, [])

  useEffect(() => {
    return () => {
      document.removeEventListener('mousedown', closeMenuMouse)
      document.removeEventListener('click', closeMenu)
      document.body.removeChild(el)
    }
  }, [])

  const render = () => {
    const { isShowReferLine, vertical, verLineArr, horLineArr, lang, menuConfigs, menuPosition } = props
    const { left, top } = menuPosition
    const isDisabled = (vertical ? !verLineArr.length : !horLineArr.length)

    return createPortal(
      <StyleMenu
        style={{ left, top }}
        showReferLine={isShowReferLine}
        lang={lang}
        menuConfigs={menuConfigs}
        id="contextMenu"
      >
        <a
          className="menu-content"
          onClick={onHandlerShowRuler}
        >
          {i18nObj[lang].show_ruler}
          {SELECT_SVG}
        </a>
        <a
          className="menu-content"
          onClick={onHandleShowReferLine}
        >
          {i18nObj[lang].show_refer_line}
          {isShowReferLine && SELECT_SVG}
        </a>
        <div className="divider" />
        <a
          className={`menu-content${isDisabled ? ' disabled' : ''}`}
          onClick={onHandleShowSpecificRuler}
        >
          {i18nObj[lang].remove_all}
          {vertical ? i18nObj[lang].horizontal : i18nObj[lang].vertical}
          {i18nObj[lang].refer_line}
        </a>
      </StyleMenu>,
      el)
  }

  return render()
}

export default memo(RulerContextMenu)
